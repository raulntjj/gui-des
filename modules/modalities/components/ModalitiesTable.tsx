...
import { useModalities, useDeleteModality } from "../services/queries"
...
export function ModalitiesTable() {
  ...
  const { data, isLoading, error } = useModalities(page, search)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Modalidade deletada com sucesso!",
      })
      setDeleteId(null)
    } catch (error) {
      toast({
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-center text-red-600">Erro ao carregar modalidades</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Gerencie as modalidades esportivas</CardDescription>
            </div>
          ...
          {isLoading ? (
            ...
            <>
                    ...
                    <TableRow key={modality.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" onClick={() => handleEdit(modality)}>
                            <Edit className="h-4 w-4" />
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => setDeleteId(modality.id)}>
              ...
              {data && data.data.length === 0 && (
                <p className="text-center py-8 text-muted-foreground">Nenhuma modalidade encontrada</p>
              )}
      ...
      <ModalityForm modality={selectedModality} open={isFormOpen} onOpenChange={setIsFormOpen} />
