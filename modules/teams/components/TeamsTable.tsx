...
export function TeamsTable() {
  ...
  const { data, isLoading, error } = useTeams(page, search)
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-center text-red-600">Erro ao carregar equipes</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <>
                ...
                <TableBody>
                  {data?.data.map((team) => (
                    <TableRow key={team.id}>
                      ...
                      <TableCell>{team.modality.name}</TableCell>
              ...
              {data && data.data.length === 0 && (
                <p className="text-center py-8 text-muted-foreground">Nenhuma equipe encontrada</p>
              )}
      ...
      <TeamForm team={selectedTeam} open={isFormOpen} onOpenChange={setIsFormOpen} />
